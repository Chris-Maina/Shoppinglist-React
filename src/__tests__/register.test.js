...
describe('Form Validation in Register page', () => {
    ...
    it('Submits to the user details to API server', () => {

        moxios.stubRequest('https://shoppinglist-restful-api.herokuapp.com/auth/register/', {
            status: 200,
            responseText: "Successful registration"
        })
        let onRegister = sinon.spy()
        axios.post('https://shoppinglist-restful-api.herokuapp.com/auth/register/').then(onRegister)
        moxios.wait( function () {
            expect(onRegister.getCall(0).args[0].data).toBe('Successful registration');
            done()
        })
    });
